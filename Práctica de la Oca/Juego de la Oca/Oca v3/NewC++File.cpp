    ...
    if (!cargaPartidas(partidas)) {
        ...
        if (opcion == 'n' || opcion == 'N') {
            ...
            if (ganador == -1) {
                ...
                if (insertado) {
                    cout << endl << "PARTIDA NO ACABADA" << endl;
                    cout << "Partida guardada correctamente con el identificador: " << id << endl;
                }
                else {
                    cout << "ERROR AL GUARDAR LA PARTIDA" << endl;
                }
...
int partida(tEstadoPartida& estado) {
    ...
    pintaTablero(estado);
    int turno = estado.turno;
    cout << "________EMPIEZA EL JUGADOR " << turno << "________" << endl;
    while (!finPartida) {

        cout << "TURNO DEL JUGADOR " << turno << endl;
        if (estado.jugadores[turno - 1].penalizaciones == 0) {
            tirada(estado.tablero, estado.jugadores[turno - 1]);
        }
        else {
            cout << "Te quedan " << estado.jugadores[turno - 1].penalizaciones << " turno/s sin jugar" << endl;
            estado.jugadores[turno - 1].penalizaciones--;
        }

        if (estado.jugadores[turno - 1].casilla < CASILLA_META && !esCasillaPremio(estado.tablero, estado.jugadores[turno - 1].casilla)) {
            turno = (turno % NUM_JUGADORES) + 1;
        ...
        if (estado.jugadores[turno - 1].casilla >= CASILLA_META - 1) {
            finPartida = true;
        }
        pintaTablero(estado);
...
void guardaPartidas(const tListaPartidas& partidas) {
    ...
    if (abierto) {
        ...
        for (int i = 0; i < partidas.contador; i++) {
            ...
            archivo << partidas.arrayPartidas[i].turno << endl;
